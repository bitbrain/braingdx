...
public class RPGScreen extends AbstractScreen<RPGTest> {
   ...
   private void prepareResources(GameContext context) {
      ...
      GameObject player = spawnObject(context, 10, 10, NPC.CITIZEN_MALE);
      ...
      for (int i = 0; i < NPCS; ++i) {
         ...
         spawnObject(context, randomX, randomY, NPC.random());
      }

      context.getLightingManager().addPointLight("lantern", 200, 200, 500, Color.valueOf("ff9955ff"));
   }

   private GameObject spawnObject(GameContext context, int indexX, int indexY, NPC type) {
      GameObject object = factory.spawn(indexX, indexY, type.ordinal());
      RasteredMovementBehavior behavior = new RasteredMovementBehavior(context.getTiledMapManager().getAPI()).interval(0.3f).rasterSize(BLOCK_SIZE,
            BLOCK_SIZE);
