...
public class PostProcessor implements Disposable {
    ...
    /**
     * Construct a new PostProcessor with the given parameters, defaulting to
     * <em>TextureWrap.ClampToEdge</em> as texture wrap mode
     */
    public PostProcessor(int fboWidth, int fboHeight, boolean useDepth, boolean useAlphaChannel, boolean use32Bits) {
	this(fboWidth, fboHeight, useDepth, useAlphaChannel, use32Bits, TextureWrap.ClampToEdge,
		TextureWrap.ClampToEdge);
    }

    /**
     * Construct a new PostProcessor with the given parameters and viewport, defaulting to
     * <em>TextureWrap.ClampToEdge</em> as texture wrap mode
     */
    ...
    /**
     * Construct a new PostProcessor with the given parameters, viewport and the specified texture
     * wrap mode
     */
    public PostProcessor(Rectangle viewport, boolean useDepth, boolean useAlphaChannel, boolean use32Bits,
     ...
     * will manage cleaning it up for you. The order of the inserted effects IS important, since
     * effects will be applied in a FIFO fashion, the first added is the first being applied.
     */
    ...
    /**
     * Starts capturing the scene as {@link #capture()}, but <strong>without</strong> clearing the
     * screen.
     *
    ...
    /**
     * Stops capturing the scene and apply the effect chain, if there is one. If the specified
     * output framebuffer is NULL, then the rendering will be performed to screen.
     */
